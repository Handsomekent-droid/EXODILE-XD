...
class CommandHandler {
      ...
      if (filename && filename.endsWith('.js')) {
        ...
        try {
          if (fs.existsSync(filePath)) {
            ...
            // Normalize unicode key before checking
            if (!plugin.command && plugin['ᴄᴏᴍᴍᴀɴᴅ']) plugin = Object.assign({}, plugin, { command: plugin['ᴄᴏᴍᴍᴀɴᴅ'] });
            if (plugin.command) {
  ...
  loadCommands() {
    ...
    for (const file of files) {
      try {
        ...
        // ᴀʀʀᴀʏ ᴇxᴘᴏʀᴛ (ᴍᴜʟᴛɪ-ᴄᴏᴍᴍᴀɴᴅ ꜰɪʟᴇ)
        const rawPlugins = Array.isArray(exported) ? exported : [exported];
        // Normalize unicode 'ᴄᴏᴍᴍᴀɴᴅ' key to ASCII 'command' for every plugin
        const plugins = rawPlugins.map(p => (p && !p.command && p['ᴄᴏᴍᴍᴀɴᴅ']) ? Object.assign({}, p, { command: p['ᴄᴏᴍᴍᴀɴᴅ'] }) : p);
        ...
        for (const plugin of plugins) {
          ...
          if (plugin.isPrefixless === true) {
            const cmdKey = plugin.command.toLowerCase();
            this.prefixlessCommands.set(cmdKey, cmdKey);
            (plugin.aliases || []).forEach(a => this.prefixlessCommands.set(a.toLowerCase(), cmdKey));
          }
        }
      } catch (error) {
  ...
  registerCommand(plugin) {
    // Normalize: plugins may use unicode small-caps 'ᴄᴏᴍᴍᴀɴᴅ' key instead of ASCII 'command'
    if (!plugin.command && plugin['ᴄᴏᴍᴍᴀɴᴅ']) {
      plugin = Object.assign({}, plugin, { command: plugin['ᴄᴏᴍᴍᴀɴᴅ'] });
    }
