...
async function isSudo(userId) {
    try {
        // Normalize: strip device suffix like :12, keep only number@s.whatsapp.net
        const normalize = j => j ? j.replace(/:\d+/, '').toLowerCase() : '';
        const normalizedUser = normalize(userId);
        const data = await loadUserGroupData();
        if (!data.sudo || !data.sudo.length) return false;
        return data.sudo.some(j => normalize(j) === normalizedUser);
    } catch (error) {
...
async function addSudo(userJid) {
    try {
        ...
        const normalize = j => j ? j.replace(/:\d+/, '').toLowerCase() : '';
        const normalizedJid = normalize(userJid);
        const data = await loadUserGroupData();
        if (!data.sudo) data.sudo = [];
        const alreadyExists = data.sudo.some(j => normalize(j) === normalizedJid);
        if (!alreadyExists) {
...
async function removeSudo(userJid) {
    try {
        ...
        const data = await loadUserGroupData();
        if (!data.sudo) data.sudo = [];
        data.sudo = data.sudo.filter(j => normalize(j) !== normalizedJid);
        await saveUserGroupData(data);
        return true;
    } catch (error) {
...
async function addWelcome(jid, enabled, message) {
    try {
        ...
        data.welcome[jid] = {
            ...
            channelId: '120363407808106286@newsletter'
        };
