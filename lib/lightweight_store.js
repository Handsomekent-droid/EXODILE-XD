...
const store = {
  ...
  bind(ev) {
    ev.on('messages.upsert', async ({ messages }) => {
      for (const msg of messages) {
        ...
        // Skip status/broadcast — no need to store these
  ...
  */
  ...
  async setBotMode(mode) {
    ...
    if (backend === 'memory') {
      ...
      try {
        const dataDir = './data'
        if (!fs.existsSync(dataDir)) fs.mkdirSync(dataDir, { recursive: true })
        fs.writeFileSync(path.join(dataDir, 'botMode.json'), JSON.stringify({ mode }, null, 2))
      } catch (e) {
        console.error(`[STORE] Failed to persist bot mode:`, e.message)
      }
    } else {
      try {
  ...
  async getBotMode() {
    if (backend === 'memory') {
      // Load from file if not in memory yet (e.g. after restart)
      if (!this._botModeLoaded) {
        this._botModeLoaded = true
        try {
          const filePath = path.join('./data', 'botMode.json')
          if (fs.existsSync(filePath)) {
            const saved = JSON.parse(fs.readFileSync(filePath, 'utf-8'))
            if (saved?.mode) this.botMode = saved.mode
          }
        } catch {}
      }
