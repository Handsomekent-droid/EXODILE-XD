...
async function loadState() {
    try {
        if (HAS_DB) {
            const state = await _ss.getSetting('global', 'mention');
...
async function saveState(state) {
    if (HAS_DB) {
        await _ss.saveSetting('global', 'mention', state);
    } else {
...
async function ensureDefaultSticker(state) {
    try {
        ...
        console.warn('ensureDefaultSticker ꜰᴀɪʟᴇᴅ:', e?.message || e);
    }
}
...
async function setMentionCommand(sock, chatId, message, isOwner) {
    if (!isOwner) return sock.sendMessage(chatId, { text: '❌ *Only ᴏᴡɴᴇʀ or Sudo can use this ᴄᴏᴍᴍᴀɴᴅ*' }, { quoted: message });
    ...
    if (type === 'text') {
        ...
        try {
            ...
            return sock.sendMessage(chatId, { text: '❌ *ꜰᴀɪʟᴇᴅ to download media*' }, { quoted: message });
        }
    }
    ...
    try {
        ...
        console.warn('cleanup previous assets ꜰᴀɪʟᴇᴅ:', e?.message || e);
    }
    ...
    try { fs.writeFileSync(outPath, buf); } catch (e) {
        console.error('error ', e);
        return sock.sendMessage(chatId, { text: '❌ *ꜰᴀɪʟᴇᴅ to save file*' }, { quoted: message });
    }
