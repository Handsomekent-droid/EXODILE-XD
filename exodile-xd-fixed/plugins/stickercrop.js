...
async function stickercropFromBuffer(inputBuffer, isAnimated) {
  ...
  const json = {
    ...
    'sticker-pack-name': settings.packname || 'Knightʙᴏᴛ',
    'emojis': ['✂️']
  };
  ...
  async handler(sock, message, args, context) {
    ...
    if (!mediaMessage) {
      await sock.sendMessage(chatId, { 
        text: 'ᴘʟᴇᴀsᴇ reply to an image/video/sticker with .crop, or send an image/video/sticker with .crop as the caption.',
        ...channelInfo
    ...
    try {
      ...
      if (!mediaBuffer) {
        await sock.sendMessage(chatId, { 
          text: 'ꜰᴀɪʟᴇᴅ to download media. ᴘʟᴇᴀsᴇ try again.',
          ...channelInfo
        }, { quoted: messageToQuote });
        return;
      }
      ...
      await new Promise((resolve, reject) => {
        exec(ffmpegCommand, (ᴇʀʀᴏʀ, stdout, stderr) => {
          if (error) {
            console.error(error);
      ...
      if (!fs.existsSync(tempOutput)) {
        throw new Error('FFmpeg ꜰᴀɪʟᴇᴅ to create output file');
      }
      ...
      console.error(error);
      await sock.sendMessage(chatId, { 
        text: 'ꜰᴀɪʟᴇᴅ to crop sticker! Try with an image.',
        ...channelInfo
      }, { quoted: messageToQuote });
    }
  },
