...
module.exports = {
    ...
    aliases: ['del', 'remove'],
    category: 'admin',
    description: 'Delete recent messages from ɢʀᴏᴜᴘ or specific user',
    ...
    async handler(sock, message, args, context = {}) {
        ...
        if (!isBotAdmin) {
            await sock.sendMessage(chatId, { 
                text: '❌ *I need to be an ᴀᴅᴍɪɴ to delete messages*' 
            }, { quoted: message });
            return;
        }
        ...
        if (countArg === null && repliedParticipant) {
            ...
            await sock.sendMessage(chatId, { 
                text: '❌ *ᴘʟᴇᴀsᴇ specify the number of messages to delete*\n\n' +
                      '*Usage:*\n' +
                      '• `.del 5` - Delete last 5 messages from ɢʀᴏᴜᴘ\n' +
        ...
        if (toDelete.length === 0) {
            const errorMsg = deleteGroupMessages 
                ? '❌ *No recent messages found in the ɢʀᴏᴜᴘ to delete*' 
