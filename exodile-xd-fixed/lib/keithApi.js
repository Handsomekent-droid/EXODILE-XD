...
// ── SEARCH ────────────────────────────────────────────────────
// Confirmed from apiskeith.top/search
...
async function keithLyrics(query) {
  // Try all 3 lyrics endpoints (1 = checking, 2 = active, 3 = checking per screenshot)
...
// ── STALKER ───────────────────────────────────────────────────
// Confirmed from apiskeith.top/stalker (13 endpoints)
...
// ── SPIRITUAL ────────────────────────────────────────────────
// Confirmed from apiskeith.top/spiritual (8 endpoints)
...
// ── EDUCATION ────────────────────────────────────────────────
// Confirmed from apiskeith.top/education (10 endpoints)
...
// ── MOVIE ─────────────────────────────────────────────────────
// Confirmed from apiskeith.top/movie (7 endpoints)
...
// ── SPORTS ───────────────────────────────────────────────────
// Confirmed from apiskeith.top/sports (40 football endpoints)
...
// ── IMAGE ─────────────────────────────────────────────────────
// Confirmed from apiskeith.top/images (8 endpoints)
...
// ── ANIME ─────────────────────────────────────────────────────
// From apiskeith.top/anime (3 endpoints, animasu.app = active)
...
// ── ADULT (18+) ───────────────────────────────────────────────
// Confirmed from apiskeith.top/adult (5 endpoints, all active)
...
async function keithYTMP3(url) {
  // Try multiple confirmed YT audio endpoints
  for (const ep of ['/download/ytmp3', '/download/ytmp3v2', '/download/ytmp3v3',
    ...
    const u = r?.result || r?.audio || r?.download || r?.link || r?.url;
...
async function keithYTMP4(url) {
  for (const ep of ['/download/ytmp4', '/download/ytmp4v2', '/download/ytmp4v3',
    ...
    if (!r) continue;
    if (typeof r === 'string' && r.startsWith('http')) return r;
    const u = r?.result || r?.video || r?.download || r?.link || r?.url;
    if (u && typeof u === 'string' && u.startsWith('http')) return u;
  }
  return null;
}
...
async function keithInstagram(url) {
  // Try posts, stories, and general IG endpoints
  for (const ep of ['/download/igdl', '/download/instagramdl',
    ...
    const u = r?.result || r?.media || r?.url || r?.download;
    if (u) return u;
  }
  return null;
}
...
async function keithSpotify(url) {
  ...
  if (!r) return null;
  if (typeof r === 'string' && r.startsWith('http')) return r;
  return r?.result || r?.audio || r?.download || r?.link || null;
}
...
async function keithFacebook(url) {
  for (const ep of ['/download/fbdl', '/download/facebookdl', '/download/facebook1', '/download/facebook2']) {
    ...
    if (!r) continue;
    if (typeof r === 'string' && r.startsWith('http')) return r;
    const u = r?.result || r?.hd || r?.sd || r?.video || r?.download;
    if (u && typeof u === 'string' && u.startsWith('http')) return u;
  }
  return null;
}
...
async function keithTwitter(url) {
  ...
  if (!r) return null;
  if (typeof r === 'string' && r.startsWith('http')) return r;
  return r?.result || r?.video || r?.url || r?.download || null;
}
...
async function keithMediaFire(url) {
  ...
  if (!r) return null;
  if (typeof r === 'string' && r.startsWith('http')) return r;
  return r?.result || r?.download || r?.link || r?.url || null;
}
...
async function keithGDrive(url) {
  ...
  if (!r) return null;
  if (typeof r === 'string' && r.startsWith('http')) return r;
  return r?.result || r?.download || r?.link || null;
}

/**
 * APK Download from APK4ALL (Keith endpoint)
 * Result shape from user's screenshot:
 * { "status": true, "creator": "Keithkeizzah", "result": {
 *   "version": "...", "developer": "...", "fileSize": "...",
 *   "redirectLink": "https://apk4all.com.im/...",
 *   "downloadLink": "https://d.apkpure.com/...",
