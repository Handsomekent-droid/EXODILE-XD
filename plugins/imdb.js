  ...
  async handler(sock, message, args, context = {}) {
    ...
    if (!text) {
      await sock.sendMessage(chatId, { 
        text: '*ᴘʟᴇᴀsᴇ provide a movie or series title.*\nExample: `.imdb Inception`', 
        quoted: message 
      });
      return;
    }
    try {
      ...
      if (!res.ok) throw new Error(`API request ꜰᴀɪʟᴇᴅ with status ${res.status}`);
...
${ratings}
      ...
      console.error(error);
      await sock.sendMessage(chatId, { 
        text: '❌ ꜰᴀɪʟᴇᴅ to fetch movie information. ᴘʟᴇᴀsᴇ try again later.', 
        quoted: message 
      });
    }
  }
