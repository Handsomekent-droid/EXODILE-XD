...
async function handleLinkDetection(sock, chatId, message, userMessage, senderId) {
    try {
        ...
        // Always delete the link message regardless of action (warn, delete, or kick)
        try {
        ...
        if (action === 'warn' || action === 'delete') {
            await sock.sendMessage(chatId, {
                text: `⚠️ *Antilink Warning*\n\n@${senderId.split('@')[0]}, posting ${linkType} links is not allowed so die !`,
                mentions: [senderId]
            });
        }
        ...
        if (action === 'kick') {
            try {
                ...
                await sock.sendMessage(chatId, {
                    text: `🚫 @${senderId.split('@')[0]} has been removed for posting ${linkType} links what a stupid🤡 kid.`,
                    mentions: [senderId]
                });
            } catch (error) {
                console.error('error');
                await sock.sendMessage(chatId, {
                    text: `⚠️ ꜰᴀɪʟᴇᴅ to remove user. Make sure the bot is an ᴀᴅᴍɪɴ.`
                });
            }
        }
...
module.exports = {
    ...
    category: 'admin',
    description: 'Prevent users from sending links in the ɢʀᴏᴜᴘ',
        ...
        if (!action) {
            ...
            await sock.sendMessage(chatId, {
                text: `*🔗 ANTILINK SETUP*\n\n` +
                      ...
                      `• Telegram\n` +
                      `• All other links\n\n` +
                      `*Note:* admins, ᴏᴡɴᴇʀ, and Sudo users are exempt.`
            }, { quoted: message });
            return;
        }
