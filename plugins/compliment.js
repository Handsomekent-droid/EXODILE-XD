    ...
    try {
      ...
      if (!userToCompliment) {
        await sock.sendMessage(chatId, {
          text: 'ᴘʟᴇᴀsᴇ mention someone or reply to their message to compliment them!'
        }, { quoted: message });
        return;
      }
      ...
      if (ᴇʀʀᴏʀ?.data === 429) {
        ...
        try {
          await sock.sendMessage(chatId, {
            text: 'ᴘʟᴇᴀsᴇ try again in a few seconds.'
          }, { quoted: message });
        } catch (retryError) {
          console.error(error);
        }
        ...
        try {
          await sock.sendMessage(chatId, {
            text: 'An ᴇʀʀᴏʀ occurred while sending the compliment.'
          }, { quoted: message });
        } catch (sendError) {
          console.error(error);
        }
