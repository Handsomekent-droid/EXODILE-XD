...
async function getBannedUsers() {
    if (HAS_DB) {
        const banned = await _ss.getSetting('global', 'banned');
        return banned || [];
    } else {
...
async function saveBannedUsers(bannedUsers) {
    if (HAS_DB) {
        await _ss.saveSetting('global', 'banned', bannedUsers);
    } else {
...
module.exports = {
  ...
  async handler(sock, message, args, context) {
    ...
    if (isGroup) {
      if (!isBotAdmin) {
        await sock.sendMessage(chatId, { 
          text: 'ᴘʟᴇᴀsᴇ make the bot an ᴀᴅᴍɪɴ to use .unban', 
          ...channelInfo 
        }, { quoted: message });
        return;
      }
      if (!isSenderAdmin && !message.key.fromMe && !senderIsOwnerOrSudo) {
        await sock.sendMessage(chatId, { 
          text: 'Only ɢʀᴏᴜᴘ admins can use .unban', 
          ...channelInfo 
        }, { quoted: message });
        return;
      }
      ...
      if (!message.key.fromMe && !senderIsOwnerOrSudo) {
        await sock.sendMessage(chatId, { 
          text: 'Only ᴏᴡɴᴇʀ/sudo can use .unban in private chat', 
          ...channelInfo 
        }, { quoted: message });
        return;
      }
    ...
    if (!userToUnban) {
      await sock.sendMessage(chatId, { 
        text: 'ᴘʟᴇᴀsᴇ mention the user or reply to their message to unban!', 
        ...channelInfo 
      }, { quoted: message });
      return;
    }
