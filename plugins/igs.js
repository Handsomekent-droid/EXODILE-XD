...
async function convertBufferToStickerWebp(inputBuffer, isAnimated, cropSquare) {
  ...
  if (isAnimated && webpBuffer.length > 1000 * 1024) {
    try {
      ...
      await new Promise((resolve, reject) => {
        exec(harsherCmd, (error) => ᴇʀʀᴏʀ ? reject(error) : resolve());
      });
  ...
  const json = {
    ...
    'sticker-pack-name': settings.packname || 'Megaʙᴏᴛ',
    'emojis': ['📸']
  };
  ...
  if (finalBuffer.length > 900 * 1024) {
    try {
      ...
      await new Promise((resolve, reject) => {
        exec(cmdSmall, (error) => ᴇʀʀᴏʀ ? reject(error) : resolve());
      });
  ...
  try {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        res.data.on('ᴇʀʀᴏʀ', reject);
      });
      return Buffer.concat(chunks);
    } catch (e2) {
      console.error('ʙᴏᴛh axios download attempts ꜰᴀɪʟᴇᴅ:', e1?.message || e1, e2?.message || e2);
      throw e2;
    }
  }
}
...
async function forceMiniSticker(inputBuffer, isVideo, cropSquare) {
  ...
  await new Promise((resolve, reject) => {
    exec(cmd, (error) => ᴇʀʀᴏʀ ? reject(error) : resolve());
  });
  ...
  async handler(sock, message, args, context) {
    ...
    try {
      ...
      if (!downloadData || !downloadData.data) {
        await sock.sendMessage(chatId, { 
          text: '❌ ꜰᴀɪʟᴇᴅ to fetch media from Instagram link.',
          ...channelInfo
        }, { quoted: message });
        return;
      }
      ...
      for (let i = 0; i < maxItems; i++) {
        try {
          ...
          console.error('error :', perItemErr);
        }
      }
      ...
      await sock.sendMessage(chatId, { 
        text: 'ꜰᴀɪʟᴇᴅ to create sticker from Instagram link.',
        ...channelInfo
      }, { quoted: message });
    }
  }
