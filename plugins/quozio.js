...
module.exports = {
  ...
  async handler(sock, message, args, context = {}) {
    ...
    try {
      ...
      if (!quoteData?.quoteId) throw new Error('Quote creation ꜰᴀɪʟᴇᴅ');
      ...
      if (!imageData?.medium) throw new Error('Image generation ꜰᴀɪʟᴇᴅ');

      await sock.sendMessage(chatId, { image: { url: imageData.medium }, caption: `📝 Quote Created\n\nAuthor: ${author}\n\n${text}` }, { quoted: message });

    } catch (error) {
      console.error(error);
      await sock.sendMessage(chatId, { text: '❌ ꜰᴀɪʟᴇᴅ to create quote. Try again later.' }, { quoted: message });
    }
  }
};
