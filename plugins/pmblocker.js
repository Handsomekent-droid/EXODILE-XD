...
const DEFAULT_MESSAGE = '⚠️ Direct messages are blocked!\nYou cannot DM this bot. ᴘʟᴇᴀsᴇ contact the ᴏᴡɴᴇʀ in ɢʀᴏᴜᴘ chats only.';

async function readState() {
    try {
        if (HAS_DB) {
            const data = await _ss.getSetting('global', 'pmblocker');
            if (!data) {
...
async function writeState(enabled, message) {
    try {
        ...
        if (HAS_DB) {
            await _ss.saveSetting('global', 'pmblocker', payload);
        } else {
