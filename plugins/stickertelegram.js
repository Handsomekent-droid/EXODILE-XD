  ...
  async handler(sock, message, args, context) {
    ...
    try {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`HTTP ᴇʀʀᴏʀ! status: ${response.status}`);
        }
        ...
        throw new Error(`ꜰᴀɪʟᴇᴅ to process sticker pack: ${error.message}`);
      }

    } catch (error) {
      console.error(error);
      await sock.sendMessage(chatId, { 
        text: '❌ ꜰᴀɪʟᴇᴅ to process Telegram stickers!\nMake sure:\n1. The URL is correct\n2. The sticker pack exists\n3. The sticker pack is public',
        ...channelInfo
      }, { quoted: message });
    }
  }
