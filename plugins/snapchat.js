...
module.exports = {
  ...
  async handler(sock, message, args, context) {
    ...
    const ᴄᴏᴍᴍᴀɴᴅPart = rawText.slice(prefix.length).trim();
    const parts = ᴄᴏᴍᴍᴀɴᴅPart.split(/\s+/);
    const url = parts.slice(1).join(' ').trim();

    if (!url) {
      return await sock.sendMessage(chatId, { 
        text: 'ᴘʟᴇᴀsᴇ provide a Snapchat Spotlight URL.\nExample: .snapchat https://www.snapchat.com/spotlight/...',
        ...channelInfo
      }, { quoted: message });
    }
    ...
    try {
      ...
      await sock.sendMessage(chatId, { 
        text: `❌ ꜰᴀɪʟᴇᴅ to fetch Snapchat media.\nᴇʀʀᴏʀ: ${error.message}`,
        ...channelInfo
      }, { quoted: message });
    }
  }
